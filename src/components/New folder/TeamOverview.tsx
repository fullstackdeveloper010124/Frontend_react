...
import { Users, Clock, AlertTriangle, CheckCircle, UserCheck, UserX, MoreVertical } from 'lucide-react';
import { teamAPI, timeEntryAPI, type TimeEntry, type TeamMember as ApiTeamMember } from '@/lib/api';
...
const convertStatus = (apiStatus: string): 'active' | 'inactive' | 'on_leave' => {
...
export const TeamOverview: React.FC<TeamOverviewProps> = ({ timeEntries }) => {
    ...
    const fetchTeamData = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          const convertedMembers: TeamMember[] = response.data.map((member: ApiTeamMember) => ({
            _id: member._id,
            name: member.name,
            email: member.email,
  ...
  const getTeamMemberStats = (userId: string) => {
    const memberEntries = timeEntries.filter(entry => entry.userId === userId);
    const totalHours = memberEntries.reduce((acc, entry) => acc + (entry.duration / 60), 0);
    const billableHours = memberEntries
      .filter(entry => entry.billable)
      .reduce((acc, entry) => acc + (entry.duration / 60), 0);
    
    return {
      totalHours: totalHours.toFixed(1),
      billableHours: billableHours.toFixed(1),
      entriesCount: memberEntries.length,
      productivity: totalHours > 0 ? ((billableHours / totalHours) * 100).toFixed(0) : '0'
    };
  };
  ...
  const onLeaveMembers = teamMembers.filter(member => member.status === 'on_leave').length;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-4 text-sm">
          <div className="flex items-center space-x-1">
            ...
            <span className="text-gray-600 dark:text-gray-400">{activeMembers} Active</span>
          </div>
          <div className="flex items-center space-x-1">
            <UserX className="w-4 h-4 text-yellow-600" />
            <span className="text-gray-600 dark:text-gray-400">{onLeaveMembers} On Leave</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {teamMembers.map((member) => {
          ...
          return (
            <div key={member._id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                ...
                <div className="flex items-center space-x-4">
                  <div className="relative">
                    ...
                    <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-gray-700 ${
                      ...
                      member.status === 'on_leave' ? 'bg-yellow-500' : 'bg-gray-400'
                    }`}></div>
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-500">
                      {member.currentProject && `Working on: ${member.currentProject}`}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-500">
                      {member.lastActive && getLastActiveText(member.lastActive)}
                    </div>
                  </div>
