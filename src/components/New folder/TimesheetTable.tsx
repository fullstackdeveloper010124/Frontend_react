    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-8 text-center">
          ...
          <span className="text-gray-500 dark:text-gray-400">Loading timesheets...</span>
        </div>
      </div>
    );
  }
            ...
            {timeEntries.length === 0 ? (
              <tr>
                <td colSpan={10} className="px-6 py-12 text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 text-lg">No time entries found</p>
                  <p className="text-gray-400 dark:text-gray-500 text-sm">Start tracking time to see entries here</p>
                </td>
              </tr>
            ) : (
              sortedEntries.map((entry) => (
                <React.Fragment key={entry._id}>
                  <tr className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                          ...
                          <span className="font-mono text-sm text-gray-900 dark:text-white">
                            {entry.duration && entry.duration > 0 
                              ...
                              : entry.startTime && entry.endTime 
                                ? formatDuration(Math.floor((new Date(entry.endTime).getTime() - new Date(entry.startTime).getTime()) / (1000 * 60)))
                                : '00:00:00'
                            }
                          </span>
                        )}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${
                        entry.billable ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400' : 
                  ...
                  {expandedEntry === entry._id && entry.description && (
                    <tr>
                      <td colSpan={10} className="px-6 py-4 bg-gray-50 dark:bg-gray-700">
